...
/obj/machinery/mill/process()
	...
	switch(milled_item.type)
		if(/obj/item/reagent_containers/food/snacks/grown/wheat)	//Wheat becomes flour
			var/obj/item/reagent_containers/food/drinks/flour/F = new(src)
			output += F
		if(/obj/item/reagent_containers/food/drinks/flour)	//Flour is still flour
			var/obj/item/reagent_containers/food/drinks/flour/F = new(src)
			output += F
		else
			error = 1
...
/obj/machinery/fermenter/process()
	...
	switch(fermenting_item.type)
		if(/obj/item/reagent_containers/food/drinks/flour)	//Flour is still flour
			var/obj/item/reagent_containers/glass/bottle/beer/B = new(src)
			output += B
		else
			error = 1
...
/obj/machinery/still
	var/list/obj/item/reagent_containers/food/input = list()
	var/list/obj/item/reagent_containers/food/output = list()
	var/obj/item/reagent_containers/food/destilling_item
	var/busy = 0
	var/progress = 0
	var/error = 0
	name = "\improper Still"
...
/obj/machinery/still/process()
	...
	switch(destilling_item.type)
		if(/obj/item/reagent_containers/glass/bottle/beer)	//Flour is still flour
			var/obj/item/reagent_containers/glass/bottle/vodka/V = new(src)
			output += V
		else
			error = 1
...
/obj/machinery/centrifuge
	var/list/obj/item/reagent_containers/food/input = list()
	var/list/obj/item/reagent_containers/food/output = list()
	var/obj/item/reagent_containers/food/spinning_item
	var/busy = 0
	var/progress = 0
	var/error = 0
	var/enzymes = 0
