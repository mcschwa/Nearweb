	...
	if(usr.client != src.owner || !check_rights(0))
		...
		switch(href_list["call_shuttle"])
			if("1")
				...
				a.autosay("Alert: The Babylon is being launched. They will launch in [round(emergency_shuttle.timeleft()/60)] minutes.", "Babylon Console")
		...
		a.autosay("Alert: The Babylon will launch in [round(emergency_shuttle.timeleft()/60)] minutes.", "Babylon Console")
...
*/
	...
	//Player Notes
		...
		if(istype(M, /mob/living/carbon/human))
			...
			observer.equip_to_slot_or_del(new /obj/item/clothing/shoes/lw/black(observer), slot_shoes)
		M.Paralyse(5)
		sleep(5)
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
		...
		if (number == 1)
			...
			message_admins("[key_name(usr)] created a [english_list(paths)]")
			for(var/path in paths)
				if(ispath(path, /mob))
			...
			message_admins("[key_name(usr)] created [number]ea [english_list(paths)]")
			for(var/path in paths)
				if(ispath(path, /mob))
