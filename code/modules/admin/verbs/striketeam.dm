...
/mob/living/carbon/human/proc/equip_death_commando(leader_selected = 0)
	...
	if (!leader_selected)
		equip_to_slot_or_del(new /obj/item/plastique(src), slot_in_backpack)
	else
	...
	equip_to_slot_or_del(new /obj/item/melee/energy/sword(src), slot_l_store)
	equip_to_slot_or_del(new /obj/item/grenade/flashbang(src), slot_r_store)
	equip_to_slot_or_del(new /obj/item/tank/emergency_oxygen(src), slot_s_store)
