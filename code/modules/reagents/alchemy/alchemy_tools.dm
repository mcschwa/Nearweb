...
/obj/item/retort/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/alchemy_powder) && !powder)
		user.drop_item()
		powder = W
		W.loc = null

	else if(istype(W, /obj/item/mortar) && !powder && !is_on)
		var/obj/item/mortar/M = W
		if(M.powder_contet)
	...
	else if(istype(W, /obj/item/flame) && !is_on)
	...
	else if(istype(W, /obj/item/reagent_containers/glass/beaker/vial))
...
/obj/item/retort/attack_hand(mob/user as mob)
	if(is_on)
		is_on = 0
...
/obj/item/retort/process()
	if(!is_on)
		processing_objects.Remove(src)
		return

	var/turf/T = locate(x+1,y,z)
	var/list/obj/item/reagent_containers/V_list = list()
	for(var/obj/item/reagent_containers/glass/V in T)
		V_list += V

	for(var/obj/item/reagent_containers/food/drinks/D in T)
		V_list += D
...
/obj/item/mortar/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/alchemy_powder) && !powder_contet)
		user.drop_item()
		powder_contet = W
		W.loc = null

	else if(istype(W, /obj/item/reagent_containers/food/snacks/grown/mushroom) && length(contents) < 4 && !powder_contet)
		contents += W
		user.drop_item()
		W.loc = src


	else if(istype(W, /obj/item/pestle) && length(contents) && !powder_contet)
...
/obj/item/mortar/attack_self(mob/user as mob)
	empty()

/obj/item/mortar/proc/empty()
	if(powder_contet)
	...
	if(length(contents))
		for(var/obj/item/reagent_containers/food/snacks/grown/mushroom/M in contents)
			contents -= M
			M.loc = get_turf(src)

	update_icon()

/obj/item/mortar/proc/fail(mob/user as mob)
...
/obj/item/mortar/proc/check_alchemy(mob/user as mob)
	if(istype(user, /mob/living/carbon))
		var/mob/living/carbon/C = user
		if(C.my_skills.get_skill(SKILL_ALCH))
