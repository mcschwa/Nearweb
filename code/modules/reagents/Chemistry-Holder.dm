...
/datum/reagents/proc/get_random_reagent_id()
	var/list/L = list()
	for(var/datum/reagent/A in reagent_list)
		L.Add(A.id)
	if(L.len)
...
/datum/reagents/proc/get_random_reagent()
	var/list/L = list()
	for(var/datum/reagent/A in reagent_list)
		L.Add(A)
	if(L.len)
...
/datum/reagents/proc/reaction(var/atom/A, var/method=TOUCH, var/volume_modifier=0, var/target_zone, var/override = 0)
	var/volumeToGo = volume_modifier
	if(override)
		volumeToGo = override
	switch(method)
		if(TOUCH)
			for(var/datum/reagent/R in reagent_list)
				...
				if(override)
					volumeToGo = override
				if(ismob(A))
					R.reaction_mob(A, TOUCH, volumeToGo)
				if(isturf(A))
					R.reaction_turf(A, volumeToGo)
				if(isobj(A))
					R.reaction_obj(A, volumeToGo)
		if(INGEST)
			for(var/datum/reagent/R in reagent_list)
				volumeToGo += R.volume
				if(override)
					volumeToGo = override
				if(ismob(A))
					R.reaction_mob(A, INGEST, volumeToGo)
				if(isturf(A))
					R.reaction_turf(A, volumeToGo)
				if(isobj(A))
					R.reaction_obj(A, volumeToGo)
		if(INJECT)
			for(var/datum/reagent/R in reagent_list)
				...
				if(override)
					volumeToGo = override
				if(ismob(A))
					R.reaction_mob(A, INJECT, volumeToGo, target_zone)
				if(isturf(A))
					R.reaction_turf(A, volumeToGo)
				if(isobj(A))
					R.reaction_obj(A, volumeToGo)
