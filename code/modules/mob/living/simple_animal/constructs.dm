...
/mob/living/simple_animal/construct/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(O.force)
		...
		for(var/mob/M in viewers(src, null))
			if ((M.client && !( M.blinded )))
				M.show_message("<span class='hitbold'>[src]</span> <span class='hit'>has been attacked with [O] by</span> <span class='hitbold'>[user]</span><span class='hit'>.</span> ")
	else
		to_chat(user, "<span class='combat'>This weapon is ineffective, it does no damage.</span>")
		for(var/mob/M in viewers(src, null))
			if ((M.client && !( M.blinded )))
				M.show_message("<span class='passivebold'>[user]</span> <span class='passive'>gently taps</span> <span class='passivebold'>[src]</span> <span class='passive'>with [O].</span>")
