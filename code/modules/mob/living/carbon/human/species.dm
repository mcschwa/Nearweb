...
/datum/species
	...
	var/teeth_type = /obj/item/stack/teeth/human //Teeth
	...
	var/list/has_organ = list(
		...
		"right eye" =     /datum/organ/internal/right_eye,
		"left eye" =     /datum/organ/internal/left_eye,
		"stomach" =     /datum/organ/internal/stomach,
		"guts" =     /datum/organ/internal/guts
		)
	var/list/not_in_storage = list(
		"brain" =    /datum/organ/internal/brain,
		"right eye" =     /datum/organ/internal/right_eye,
		"left eye" =     /datum/organ/internal/left_eye
	)
...
/datum/species/proc/create_organs(var/mob/living/carbon/human/H) //Handles creation of mob organs.
	...
	if(H.organ_storage)             H.organ_storage = null
	...
	if(!H.height)
		if(H.gender == FEMALE)
			...
			H.height = rand(minheightm, maxheightm)

	//This is a basic humanoid limb setup.
	H.organs_by_name["chest"] = new/datum/organ/external/chest()
	H.organs_by_name["vitals"] = new/datum/organ/external/vitals(H.organs_by_name["chest"])
	H.organs_by_name["throat"] = new/datum/organ/external/throat(H.organs_by_name["chest"])
	H.organs_by_name["groin"] = new/datum/organ/external/groin(H.organs_by_name["chest"])
	H.organs_by_name["head"] = new/datum/organ/external/head(H.organs_by_name["throat"])
	...
	for(var/organ in has_organ)
		...
		var/datum/organ/internal/I = new organ_type(H)
...
/mob/living/carbon/human/proc/isChild()//Used to tell if someone is a child.
	if(species && (species.name == "Child" || species.name == "Zombie Child"))
		return 1
	else
		return 0

/mob/living/carbon/human/proc/isMidget()
	if(species && species.name == "Midget")
		return 1
	else
		return 0
...
/datum/species/human/skeleton
	...
	total_health = 50
	insulated = 1
	flags = NO_BREATHE|NO_SCAN|NO_BLOOD|NO_PAIN
	has_organ = list()

/datum/species/human/skeleton/handle_post_spawn(var/mob/living/carbon/human/H)
	. = ..()
	H.see_invisible = 15
	H.see_in_dark = 100
	H.vessel.del_reagent("blood")
	//If you wanted to add a species-level ability:
	/*abilities = list(/client/proc/test_ability)*/
...
/datum/hud_data
	...
	var/list/gear = list(
		"i_clothing" =   list("loc" = ui_iclothing, "slot" = slot_w_uniform, "state" = "center", "dir" = SOUTH),
		"o_clothing" =   list("loc" = ui_oclothing, "slot" = slot_wear_suit, "state" = "equip",  "dir" = SOUTH),
		"mask" =         list("loc" = ui_mask,      "slot" = slot_wear_mask, "state" = "equip",  "dir" = NORTH),
		"gloves" =       list("loc" = ui_gloves,    "slot" = slot_gloves,    "state" = "gloves"),
		"eyes" =         list("loc" = ui_glasses,   "slot" = slot_glasses,   "state" = "glasses"),
		"l_ear" =        list("loc" = ui_l_ear,     "slot" = slot_l_ear,     "state" = "ears"),
		"r_ear" =        list("loc" = ui_r_ear,     "slot" = slot_r_ear,     "state" = "ears"),
		"wrist_r" =      list("loc" = ui_wrist_r,   "slot" = slot_wrist_r,   "state" = "wrist_r"),
		"wrist_l" =      list("loc" = ui_wrist_l,   "slot" = slot_wrist_l,   "state" = "wrist_l"),
		"amulet" =       list("loc" = ui_amulet ,   "slot" = slot_amulet,    "state" = "amulet"),
		"back2" =        list("loc" = ui_back2 ,    "slot" = slot_back2,     "state" = "back2"),
		"head" =         list("loc" = ui_head,      "slot" = slot_head,      "state" = "hair"),
		"shoes" =        list("loc" = ui_shoes,     "slot" = slot_shoes,     "state" = "shoes"),
		"belt2" =        list("loc" = ui_sstore1,   "slot" = slot_s_store,   "state" = "belt",   "dir" = 8),
