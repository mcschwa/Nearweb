...
/obj/machinery/portable_atmospherics/hydroponics
	...
	var/cycledelay = 80       // Delay per cycle.
...
/obj/machinery/portable_atmospherics/hydroponics/examine()
	..()
	if(dead)
		to_chat(usr, "<span class='passive'>The plant is dead.</span>")
		return
	if(weedlevel > 0)
		to_chat(usr, "<span class='passive'>crops looks ill.</span>")
	if(waterlevel <= 10)
		to_chat(usr, "<span class='passive'>it needs some water.</span>")
	if(nutrilevel <= 2)
		to_chat(usr, "<span class='passive'>it needs some nutrition.</span>")
	if(harvest)
		to_chat(usr, "<span class='passivebold'>It's ready for harvest.</span>")
/obj/machinery/portable_atmospherics/hydroponics/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/wirecutters) || istype(O, /obj/item/surgery_tool/scalpel))
		...
		if(!seed)
			...
			playsound(src, 'sound/effects/seedsplanted.ogg', 70, 0, -6)
		...
		if(weedlevel > 0)
			...
			weedlevel = 0
			update_icon()
			playsound(O, pick('sound/weapons/pfork01.ogg','sound/weapons/pfork02.ogg','sound/weapons/pfork03.ogg'), 70, 0, -6)
		else
...
/obj/machinery/portable_atmospherics/hydroponics/soil/New()
	...
	icon_state = pick("dirt1","dirt2","dirt3","dirt4","dirt5","dirt6","dirt7","dirt8","dirt9","dirt10","dirt11","dirt12","dirt13","dirt14","dirt15","dirt16")
	density = 0
