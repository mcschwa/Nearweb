...
/obj/item/clothing/head/helmet/lw/elitehelmet/verb/toggle()
	...
	if(usr.canmove && !usr.stat && !usr.restrained())
		if(src.up)
			...
			flags_inv |= HIDEEYES
			icon_state = "hero0"
			to_chat(usr, "You flip the [src] visor down.")
			...
			flags_inv &= ~HIDEEYES
			icon_state = "hero1"
			to_chat(usr, "You push the [src] visor up out of your face.")
			body_parts_covered = HEAD
			flags_inv = HIDEEARS|HIDEEYES
...
/obj/item/clothing/head/helmet/lw/mardukwowhelm  //Horned Helmet
	name = "marduk helmet"
	desc = "An ornate horned helmet worn by a captain of the Tiamathi Guard."
	icon_state = "censorhorns"
	item_state = "helmet"
	flags = FPRINT|TABLEPASS|HEADCOVERSEYES|BLOCKHAIR
	body_parts_covered = HEAD|FACE|MOUTH
...
/obj/item/clothing/head/helmet/lw/mardukhelm  // Non-Horned Helmet
	name = "marduk helmet"
	desc = "An ornate helmet worn by a captain of the Tiamathi Guard."
	icon_state = "censor"
	item_state = "helmet"
	flags = FPRINT|TABLEPASS|HEADCOVERSEYES|BLOCKHAIR
	body_parts_covered = HEAD|FACE|MOUTH
...
/obj/item/clothing/head/helmet/sechelm/veteran
	name = "\the Marduk's helmet"
	desc = "The unique helemt of the leader of the Tiamat."
	shockable = FALSE
	icon_state = "sechelmvet"


/obj/item/clothing/head/helmet/sechelm/examine(mob/user)
	..()
	if(camera)
		to_chat(user, "<span class='combat'><small>\The [src]'s camera is on!</small></span>")
