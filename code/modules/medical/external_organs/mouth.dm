...
/datum/organ/external/mouth/proc/knock_out_teeth(throw_dir, num=32, var/set_num = null) //Won't support knocking teeth out of a dismembered head or anything like that yet.
	num = Clamp(num, 1, 32)
	var/done = 0
	if(teeth_list && teeth_list.len) //We still have teeth
		var/stacks = rand(1,3)
		if(set_num)//For the teeth padla.
			stacks = rand(3,set_num)
