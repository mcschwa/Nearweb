...
obj/machinery/hydroponics/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	//		Toxicity dilutation code. The more water you put in, the lesser the toxin concentration.
		...
		if(!planted)
			...
			playsound(O, 'sound/effects/seedsplanted.ogg', 80, 0, -6)
			dead = 0
			myseed = O
			planted = 1
			age = 1
		...
		if(weedlevel > 0)
			...
			playsound(O, pick('sound/weapons/pfork01.ogg','sound/weapons/pfork02.ogg','sound/weapons/pfork03.ogg'), 80, 0, -6)
			weedlevel = 0
			updateicon()
			src.updateicon()
		else
