...
/obj/machinery/computer/arcade/New()
	...
	name_action = pick("Defeat ", "Annihilate ", "Strike ", "Stop ", "Destroy ", "Kill ", "Rape ")

	name_part1 = pick("Pidron ", "Lord ", "the Evil ", "King ", "Lord ", "the Great ", "Duke ", "General ")
	name_part2 = pick("Thanati", "Graga", "Kthanid", "Tzchernobog")
	...
	if ((src.enemy_mp <= 0) || (src.enemy_hp <= 0))
		...
		if (src.player_mp <= 0)
			...
			if(ishuman(usr))
				...
				H.add_event("arcade", /datum/happiness_event/misc/lost)
			if(emagged)
				usr.gib()
