...
/proc/call_shuttle_proc(var/mob/user)
	...
	if(evac_type == "pods")
		a.autosay("Alert: The Babylon is being launched. They will launch in [round(emergency_shuttle.timeleft()/60)] minutes.", "Escape Computer")
	else
...
/obj/item/circuitboard/communications/Destroy()
	...
	message_admins("All the AIs, comm consoles and boards are destroyed. Pods launch started.", 1)
	if(evac_type == "pods")
		a.autosay("Alert: The escape pods are being launched. They will launch in [round(emergency_shuttle.timeleft()/60)] minutes.", "Babylon Console")
	else
		a.autosay("Alert: The emergency shuttle has been called. It will arrive in [round(emergency_shuttle.timeleft()/60)] minutes.", "Babylon Console")
	world << sound('sound/AI/shuttlecalled.ogg')
