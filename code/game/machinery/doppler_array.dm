...
/obj/machinery/doppler_array/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/wrench))
		...
		anchored = !anchored
		power_change()
		user << "You [anchored ? "wrenched" : "unwrenched"] [src]."
...
/obj/machinery/doppler_array/proc/sense_explosion(var/x0,var/y0,var/z0,var/devastation_range,var/heavy_impact_range,var/light_impact_range,var/took)
	...
	var/message = "Explosive disturbance detected - Epicenter at: grid ([x0],[y0],[z0]). Epicenter radius: [devastation_range]. Outer radius: [heavy_impact_range]. Shockwave radius: [light_impact_range]. Temporal displacement of tachyons: [took]seconds."
