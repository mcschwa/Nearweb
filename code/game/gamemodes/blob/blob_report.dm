...
/datum/game_mode/blob/send_intercept(var/report = 1)
	...
	switch(report)
		if(1)
			...
			intercepttext += "Reports indicate the probable transfer of a biohazardous agent onto [vessel_name()] during the last crew deployment cycle.<BR>"
			intercepttext += "Preliminary analysis of the organism classifies it as a level 5 biohazard. Its origin is unknown.<BR>"
			intercepttext += "NanoTrasen has issued a directive 7-10 for [vessel_name()]. The station is to be considered quarantined.<BR>"
			intercepttext += "Orders for all [vessel_name()] personnel follows:<BR>"
		...
		if(2)
			...
			intercepttext += "<FONT size = 3><B>NanoTrasen Update</B>: Biohazard Alert.</FONT><HR>"
			intercepttext += "Directive 7-12 has been issued for [vessel_name()].<BR>"
