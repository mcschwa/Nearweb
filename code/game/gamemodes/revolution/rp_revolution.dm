...
/datum/game_mode/revolution/proc/is_convertible(mob/M)
	for(var/obj/item/implant/loyalty/L in M)//Checking that there is a loyalty implant in the contents
		if(L.imp_in == M)//Checking that it's actually implanted
			return 0
	for(var/obj/item/implant/mentor/L in M)//Checking that there is a loyalty implant in the contents
		if(L.imp_in == M)//Checking that it's actually implanted
			return 0
...
/datum/game_mode/revolution/rp_revolution/process()
	...
	if(last_command_report == 0 && world.time >= 10 * 60 * 10)
		src.command_report("We are regrettably announcing that your performance has been disappointing, and we are thus forced to cut down on financial support to your [vessel_type]. To achieve this, the pay of all personnal, except the Heads of Staff, has been halved.")
		last_command_report = 1
	else if(last_command_report == 1 && world.time >= 10 * 60 * 30)
		src.command_report("Statistics hint that a high amount of leisure time, and associated activities, are responsible for the poor performance of many of our [vessel_type]s. You are to bolt and close down any leisure facilities, such as the holodeck, the theatre and the bar. Food can be distributed through vendors and the kitchen.")
