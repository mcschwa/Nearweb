...
/turf/simulated/wall/r_wall/New()
	..()
	if(istype(src, /turf/simulated/wall/r_wall/cave)) return
	if(istype(src, /turf/simulated/wall/r_wall/charon)) return
	if(istype(src, /turf/simulated/wall/r_wall/hell)) return
	if(istype(src, /turf/simulated/wall/r_wall/keep)) return
	if(istype(src, /turf/simulated/wall/r_wall/keepss13)) return
	if(istype(src, /turf/simulated/wall/r_wall/leviathan)) return
	if(istype(src, /turf/simulated/wall/r_wall/metal)) return
	if(istype(src, /turf/simulated/wall/r_wall/rust)) return

	var/image/I = image(icon='icons/turf/walls.dmi',icon_state="scratch[rand(1,4)]", layer = 5)
	I.pixel_x = rand(-4, 8)
	I.pixel_y = rand(-4, 8)
	overlays += I
	if(prob(14))
		var/image/II = image(icon='icons/turf/walls.dmi',icon_state="wound[rand(1,4)]", layer = 5)
		II.pixel_x = rand(-4, 8)
		II.pixel_y = rand(-4, 8)
		overlays += II
