...
/obj/item/clothing/suit/storage/proc/standard_orient_objs(var/rows,var/cols)
	var/cx = 4
	var/cy = 4
	src.boxes.screen_loc = text("4:16,4:16 to [4+cols]:16,4:16")
	//src.boxes.screen_loc = text("4:16,4:16 to 5:16,4:16")
	for(var/obj/O in src.contents)
		...
		if (cx > (4+cols))
			cx = 4
			cy--
	src.closer.screen_loc = text("[4+cols+1]:16,4:16")
	return
