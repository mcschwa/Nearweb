...
/obj/item/storage/forge/bigForge
	name = "forge"
...
/obj/item/storage/forge/bigForge/update_icon()
	if(on)
		icon_state = "furnace1"
...
/obj/item/storage/forge/bigForge/attackby(obj/item/W as obj, mob/user as mob)
	if(on)
		return

	if(istype(W, /obj/item/ore/lw) && !(src.contents.len >= storage_slots))             // SLOT CARVAO
		var/obj/item/ore/lw/L = W
		user.drop_item(sound = 0)
		src.contents += L

	if(istype(W, /obj/item/flame) && src.contents.len >= 2)
		var/COAL,IRON = 0
		on = 1
		update_icon()
		for(var/obj/item/ore/lw/L in src.contents)
			if(istype(L, /obj/item/ore/lw/coal))
				COAL += 1
			if(istype(L, /obj/item/ore/lw/ironlw))
				IRON += 1
		while(on && COAL ==1 )
			sleep(10)
			tocomplete += 10
			...
			if(tocomplete >= 160)
				if(IRON == 3)
					for(var/i = 1; i <= 4; i++)
						var/obj/item/ore/refined/lw/steellw/S = new(src.loc)
						sleep(2)
					...
					for(var/i = 1, i <= src.contents.len, i++)
						...
						if(!istype(contentes, /obj/item/ore/lw/coal))
							var/obj/item/ore/lw/A = contentes
