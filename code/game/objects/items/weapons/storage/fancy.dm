...
/obj/item/storage/fancy/vials/attackby(obj/item/W as obj, mob/user as mob)
	if(!locked)
		..()
	else
...
/obj/item/storage/fancy/vials/RightClick(mob/user)
	locked = !locked
	update_icon()

/obj/item/storage/fancy/vials/update_icon(var/itemremoved = 0)
	var/total_contents = src.contents.len - itemremoved
	src.icon_state = "vialbox[total_contents]"
	src.overlays.Cut()
	if(locked)
		overlays += image(icon, src, "vialboxcover")

/obj/item/storage/fancy/vials/show_to(mob/user as mob)
	if (locked)
		to_chat(user,"<span class ='combat'> It's locked!</span>")
	else
		..()
