...
/obj/item/surgery_tool/scalpel/laser1
	name = "laser scalpel"
	desc = "A scalpel augmented with a directed laser, for more precise cutting without blood entering the field.  This one looks basic and could be improved."
	icon_state = "scalpel_laser1_on"
	damtype = "fire"

/obj/item/surgery_tool/scalpel/laser2
	name = "laser scalpel"
...
/obj/item/surgery_tool/scalpel/laser3
	name = "laser scalpel"
...
/obj/item/surgery_tool/scalpel/manager
...
/obj/item/surgery_tool/speculum
	name = "speculum"
	desc = "Retracts stuff."
	icon_state = "speculum"
	m_amt = 10000
...
/obj/item/surgery_tool/suture/examine()
	..()
	to_chat(usr, "<span class='passive'>[amount] uses left.</span>")

/obj/item/surgery_tool/suture/proc/amountcheck()
	if(amount <= 0)
		qdel(src)

/obj/item/surgery_tool/suture/attack(mob/living/carbon/human/H as mob, mob/living/userr, var/target_zone)//All of this is snowflake because surgery is broken.
	..()
	if(!ishuman(H))
		return ..()
	if(!ishuman(userr))
	...
	if((affected.get_lost_fingers_number() > 0 && affected.has_finger)  && skillcheck(user.my_skills.get_skill(SKILL_SURG), 4, 0, user))
		...
		for(var/obj/item/organ/finger/F in view(1))
...
/obj/item/surgery_tool/disinfectant
	name = "disinfectant"
	icon_state = "disinfectant"
	var/uses = 4

/obj/item/surgery_tool/disinfectant/examine()
	..()
	if(uses)
...
/obj/item/desfibrilador/New()
	..()
	CELL = new /obj/item/cell/crap/leet

/obj/item/desfibrilador/MouseDrop(var/obj/over_object)
	var/mob/user = usr
...
/obj/item/desfibrilador/attackby(var/obj/item/A, var/mob/user)
...
/obj/item/desfibrilador/attack(mob/living/carbon/human/H as mob, mob/living/carbon/human/user, var/target_zone)
...
/obj/item/bloodinjector
	name = "Blood injector"
	icon = 'icons/obj/surgery.dmi'
	icon_state = "nanoblood"
	var/obj/item/blood_catridge/catridge

/obj/item/bloodinjector/New()
	..()
	catridge = new /obj/item/blood_catridge(src)

/obj/item/bloodinjector/attack(mob/living/carbon/human/H as mob, mob/living/user, var/target_zone)
	if(ishuman(H))
...
/obj/item/bloodinjector/RightClick(mob/user)
	if(catridge)
...
/obj/item/bloodinjector/attackby(var/obj/item/A, var/mob/user)
...
/obj/item/bloodinjector/examine()
	..()
	if(catridge)
...
/obj/item/scissors/attack(mob/living/carbon/human/H as mob, mob/living/carbon/human/user, var/target_zone)
	if(!target_zone)
