...
/obj/item/device/radio/headset/receive_range(freq, level)
	if(ishuman(src.loc))
		...
		if(H.l_ear == src || H.r_ear == src || H.wrist_r == src || H.wrist_l == src || H.amulet == src)
			return ..(freq, level)
	return -1
...
/obj/item/device/radio/headset/tribunal
	origin_tech = "syndicate=3"
	icon_state = "syndie_headset"
	item_state = "syndie_headset"
	freerange = 1
...
/obj/item/device/radio/headset/tribunal/New()
	..()
	qdel(keyslot1)
