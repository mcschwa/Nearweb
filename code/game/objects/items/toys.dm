...
/obj/item/toy/pumpkinjack/RightClick(mob/user as mob)
	if(cooldown)
		return
	cooldown = TRUE
...
/obj/item/toy/duck/bite_act(mob/M as mob)
	if(inflated)
		return
	if(get_dist(src,M) >= 2)
		return
	if(do_after(M,10))
		to_chat(M, "<span class='passive'>You fill the inflatable duck.</span>")
		inflated = TRUE
		icon_state = "duck1"
		item_state = "duck"
