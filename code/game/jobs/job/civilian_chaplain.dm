...
/datum/job/inquisitor
	...
	department_flag = ENGSEC
	faction = "Station"
	stat_mods = list(STAT_ST = 2, STAT_DX = 2, STAT_HT = 2, STAT_IN = 3)
	total_positions = 1
	spawn_positions = 1
	...
	list(SKILL_UNARM,1,3),
	list(SKILL_KNIFE,1,3),
	list(SKILL_SWING,1,3),
	list(SKILL_FARM,0),
	list(SKILL_COOK,0),
	list(SKILL_ENGINE,0),
	list(SKILL_SURG,3,3),
...
/datum/job/practicus
	...
	department_flag = ENGSEC
	faction = "Station"
	stat_mods = list(STAT_ST = 3, STAT_DX = 0, STAT_HT = 2, STAT_IN = 0)
	total_positions = 2
	spawn_positions = 2
	...
	equip(var/mob/living/carbon/human/H)
		...
		if(Inquisitor_Type == "Holy War Veterans")
			...
			H.equip_to_slot_or_del(new /obj/item/melee/telebaton(H), slot_r_store)
			H.add_perk(/datum/perk/ref/strongback)
			H.add_perk(/datum/perk/morestamina)
			H.terriblethings = TRUE
			H.my_stats.change_stat(STAT_ST , 1)
