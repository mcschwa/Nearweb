...
*/
	...
	var/found = 0
	for(var/obj/machinery/computer/rdconsole/RDC in range(src,7))
		if(!RDC.sync)
			continue
		found++
...
/obj/machinery/mecha_part_fabricator/Topic(href, href_list)
	...
	if(href_list["part_set"])
		var/tpart_set = d_filter.getStr("part_set")
		if(tpart_set)
			if(tpart_set=="clear")
				src.part_set = null
			else
				...
				screen = "parts"
	if(href_list["part"])
		var/list/part = d_filter.getObj("part")
		if(!processing_queue)
			build_part(part)
		else
			add_to_queue(part)
	if(href_list["add_to_queue"])
		add_to_queue(d_filter.getObj("add_to_queue"))
		return update_queue_on_page()
	if(href_list["remove_from_queue"])
		remove_from_queue(d_filter.getNum("remove_from_queue"))
		return update_queue_on_page()
	if(href_list["partset_to_queue"])
		add_part_set_to_queue(d_filter.get("partset_to_queue"))
		return update_queue_on_page()
