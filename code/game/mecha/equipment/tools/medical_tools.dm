/obj/item/mecha_parts/mecha_equipment/tool/sleeper
	...
	Topic(href,href_list)
		...
		if(d_filter.get("eject"))
			go_out()
		if(d_filter.get("view_stats"))
			chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
			onclose(chassis.occupant, "msleeper")
			return
		if(d_filter.get("inject"))
			inject_reagent(d_filter.getType("inject",/datum/reagent),d_filter.getObj("source"))
		return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun
	...
	Topic(href,href_list)
		..()
		var/datum/topic_input/d_filter = new (href,href_list)
		if(d_filter.get("toggle_mode"))
			mode = !mode
			update_equip_info()
			return
		if(d_filter.get("select_reagents"))
			...
			for(var/i=1 to known_reagents.len)
				if(m>=synth_speed)
					break
				var/reagent = d_filter.get("reagent_[i]")
			...
			return
		if(d_filter.get("show_reagents"))
			chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
		if(d_filter.get("purge_reagent"))
			var/reagent = d_filter.get("purge_reagent")
			if(reagent)
				reagents.del_reagent(reagent)
			return
		if(d_filter.get("purge_all"))
