...
/obj/mecha/proc/check_for_support()
	if(locate(/obj/structure/grille, orange(1, src)) || locate(/obj/structure/lattice, orange(1, src)) || locate(/obj/structure/catwalk, orange(1, src))|| locate(/turf/simulated, orange(1, src)) || locate(/turf/unsimulated, orange(1, src)))
		return 1
	else
		return 0
...
/obj/mecha/Topic(href, href_list)
	...
	if(href_list["select_equip"])
		if(usr != src.occupant)	return
		var/obj/item/mecha_parts/mecha_equipment/equip = d_filter.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rfreq"])
		if(usr != src.occupant)	return
		var/new_frequency = (radio.frequency + d_filter.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		...
		output_access_dialog(d_filter.getObj("id_card"),d_filter.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))	return
		var/mob/user = d_filter.getMob("user")
		if(user)
			if(state==0)
				state = 1
			...
			output_maintenance_dialog(d_filter.getObj("id_card"),user)
		return
	...
	if(href_list["add_req_access"] && add_req_access && d_filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access += d_filter.getNum("add_req_access")
		output_access_dialog(d_filter.getObj("id_card"),d_filter.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && d_filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access -= d_filter.getNum("del_req_access")
		output_access_dialog(d_filter.getObj("id_card"),d_filter.getMob("user"))
		return
	...
	if(href_list["debug"])
		if(href_list["set_i_dam"])
			setInternalDamage(d_filter.getNum("set_i_dam"))
		if(href_list["clear_i_dam"])
			clearInternalDamage(d_filter.getNum("clear_i_dam"))
		return
	*/
