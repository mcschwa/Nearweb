	...
	for(var/obj/effect/landmark/L in landmarks_list)
		if(L.name == "Response Team")
			...
			spawn(0)
				...
				// -- CHANGE APPEARANCE --
				var/new_tone = input(new_member, "Please select your new skin tone: 1-255 (1=albino, 35=caucasian, 150=black, 220='very' black)", "Character Generation") as num

				if(new_tone)
					new_member.s_tone = max(min(round(text2num(new_tone)), 255), 1)
...
/mob/living/carbon/human/proc/equip_response_team(leader_selected = 0)
	...
	// equip_to_slot_or_del(new /obj/item/storage/firstaid/regular(src), slot_in_backpack) // Regular medkit
