...
/obj/machinery/shielding/energyconverter/interact(mob/user as mob)
...
/obj/machinery/shielding/energyconverter/update_icon()
	src.overlays.Cut()
	if (stat)
		icon_state = "econ-p"
	else
		...
		if(OperatingMode && Capacitor && !Capacitor.stat)
			overlays += image('shieldgen.dmi', "o[round((ConversionRate * 11) / HardMaximumConversionRate)]")
		else
			overlays += image('shieldgen.dmi', "econ-o")